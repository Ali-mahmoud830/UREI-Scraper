...
export default function AdminPanel() {
  ...
  // Forms
  const [newToken, setNewToken] = useState({ email: "", tier: "monthly", days: 30 });
  ...
  const handleCreateToken = async (e: React.FormEvent) => {
    e.preventDefault();
    // Optimistic update: push a skeleton user into the list immediately
    const optimisticUser = { email: newToken.email, tier: newToken.tier, total_searches: 0, ip_address: "pending...", token_code: "generating...", expires_at: null };
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Refresh to replace optimistic with real data
        fetchUsers(password);
      } else {
        // Rollback optimistic
        setUsers(prev => prev.filter(u => u.email !== newToken.email || u.ip_address !== "pending..."));
        toast.error(data.detail || "Error generating token");
        addLog(`ERROR: Token mint FAILED for ${newToken.email} — ${data.detail}`);
      }
    } catch {
      setUsers(prev => prev.filter(u => u.ip_address !== "pending..."));
                ...
                <div className="flex-1 pr-6">
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      value={config?.FREE_RESULT_LIMIT ?? 50}
                      onChange={(e) => setConfig({ ...config, FREE_RESULT_LIMIT: e.target.value })}
            ...
            <form onSubmit={handleCreateToken} className="space-y-5">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="w-1/2 relative">
                    <input
                      type="number" min="1"
                      value={newToken.days}
                      onChange={(e) => setNewToken({ ...newToken, tier: 'custom', days: parseInt(e.target.value) || 1 })}
              ...
              <div className={`border rounded-xl p-4 space-y-2 transition-all bg-black/20 ${tierInfo.color.split(" ")[1] || "border-slate-700"}`}>
                ...
                <div className="flex items-center gap-2 text-xs text-slate-400 pt-1 border-t border-slate-800 mt-2">
                  <BarChart2 className="w-3 h-3" /> Search Cap: <span className="text-white font-semibold">{tierInfo.limit}</span>
                </div>
