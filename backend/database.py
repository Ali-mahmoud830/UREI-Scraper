...
SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://uzdhrsfuzrumfglpuckz.supabase.co")
# Use the service role key for full DB access (bypasses Row Level Security)
...
class DBManager:
    ...
    def add_lead(self, phone: str, price: str, location: str, url: str,
                 session_id: int = None, intent: str = "seller") -> bool:
        try:
            # Upsert by phone (unique)
            res = self.sb.table("leads").upsert(
                {"phone": phone, "price": price, "location": location, "url": url,
    ...
    def create_session(self, city: str, property_type: str,
        ...
        try:
            res = self.sb.table("search_sessions").insert({
    ...
    def get_sessions(self):
        try:
            res = self.sb.table("search_sessions").select("*").order("created_at", desc=True).execute()
    ...
    def get_user_by_session(self, session_key: str):
        try:
            ...
            logger.error(f"Error fetching user by session: {e}")
            return None
    ...
    def bind_token_to_session(self, token_id: int, session_key: str,
                              ip_address: str, email: str = None) -> bool:
        try:
            ...
            logger.error(f"Error binding token to session: {e}")
            return False
